"""
Registration serializers for user registration and validation.
"""
import re
import logging
...
# Import logger from parent module to match test mocking
from . import logger
...
class UserRegistrationSerializer(serializers.ModelSerializer):
    ...
    def save(self, **kwargs):
        """
        Override save to handle request context.
        """
        request = kwargs.pop('request', None)
        if request:
            self.context['request'] = request

        # Check if this is a create operation (instance is None before save)
        is_create = self.instance is None

        # Call parent save which will call our create method
        user = super().save(**kwargs)

        # Ensure logging happens for new user creation
        if is_create:
            # Import logger here to ensure it matches the test mock
            from authentication.serializers import logger as auth_logger
